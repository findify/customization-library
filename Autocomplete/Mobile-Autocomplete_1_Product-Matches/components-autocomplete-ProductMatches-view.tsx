...
// create an enhancer to check if the view is mobile or desktop
...
const ProductMatchesView: React.SFC<IProductMatchesProps> = ({
  ...
  <div className={styles.root}>
    <ItemsList
      ...
      className={className}
      // This adjustment makes sure to only display two product matches on mobile but to keep the dashboard setting for desktop.
      limit={isMobile ? 2 : config.getIn(['meta', 'item_limit'])}
      theme={theme}
      config={config}
      factory={ProductCard}
    />
...
// add the enhancer
export default enhancer(ProductMatchesView)
